...
void drawDeckControlsMode() {
  ...
  for (int d = 0; d < 2; d++) {
    ...
    drawToggleButton(x, DC_ROW_Y0, DC_COL_W, DC_ROW_H, "MASTER TEMPO", THEME_PRIMARY, deckTempo[d]);
    drawToggleButton(x, DC_ROW_Y0 + (DC_ROW_H + DC_ROW_GAP), DC_COL_W, DC_ROW_H, "QUANTIZE", THEME_ACCENT, deckQuantize[d]);
    drawToggleButton(x, DC_ROW_Y0 + 2 * (DC_ROW_H + DC_ROW_GAP), DC_COL_W, DC_ROW_H, "SLIP MODE", THEME_WARNING, deckSlip[d]);
  }
}
